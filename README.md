...
Böylece Github ve Git'in temel işlevlerini ve kullanımını daha iyi anladım. Artık projelerimi daha düzenli bir şekilde yönetebilir ve takım arkadaşlarımla daha iyi birşekilde işbirliği yapabilirim.  
...
Cisco 2950 switch denemek amaçlı temel bakımları yapılıp açıldı
05.04
osı modeli tekrarı
...
1-) GET: Bu metod sunucudan veri almak için kullanılır. GET ve POST metodları en sık kullanılan metodlar olup sunucudaki kaynaklara erişmek için kullanılırlar.GET metodu ile sorgu metinleri URL içinde gönderilebilir.

2-)POST: Bu metod ile sunucuya veri yazdırabilirsiniz. Bu metodla istek parametreleri hem URL içinde hem de mesaj gövdesinde gönderilebilir. Sadece mesaj gövdesinin kullanımı yukarıda sayılan riskleri engelleyecektir.  Bu özellik ve de güvenlik gerekçeleriyle bir işlem gerçekleştirileceğinde POST metodunun kullanılması önerilir.
...
port hızları normal portlar ile kenar portların hızı 
...
birden fazla switch arasından yükün en çok olan switch bakbone switchdir
Ağ üzerindeki geçiş yollarını oluşturur
DELL SWITCH KOMUTLARI VE ANLAMLARI
DELL SWITCH 'de PORT ARAYÜZÜNE GİRİP PORTU AÇIP KAPATMA

en // priviled moda geçer..
enable  // user-exec mode'dan çıkar. Yetkili adam moduna geçer

show interface status  // portları görüntüle
conf t // configurasyon moda geç
interface gigabitethernet 1/0/24
show interface
show interfaces gigabitethernet 1/0/24  /// 24. Portun arayüzüne girer
______________________________

interface gigabitethernet 1/0/31  // Portun arayüzüne girme
...
05.05
sistem odası bağlantıları,
server bağlantıları,
bilgiişlem eğitimleri,
it eğitimleri,
sunucu bağlantıları,
server storage bağlantıları,
firewall bağlantıları,
modem firewall bağlantısı,
server switch bağlantıları,
omurga switch bağlantıları,
sistem alt yapısı,
omurga switch,
kenar switch,
network eğitimi,
